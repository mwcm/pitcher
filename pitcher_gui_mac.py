...
def write_audio_segment(f, x, sr, normalized=False, file_type='mp3'):
    """numpy array to MP3"""
    ...
    song.export(f, format=file_type, bitrate="320k")
    return song
...
def pitch(st, input_file, output_file, log_level='INFO', quantize_bits=12, skip_normalize=False,
    ...
    if '.mp3' in output_file:
        write_audio_segment(output_file, output, OUTPUT_SR, not skip_normalize, 'mp3')
    elif '.wav' in output_file:
        write_audio_segment(output_file, output, OUTPUT_SR, not skip_normalize, 'wav')
    else:
...
if __name__ == '__main__':
    ...
    current_bits_value = tk.DoubleVar()
    current_st_value.set(0)
    current_bits_value.set(12)
    
    def get_current_st_value():
        return '{: .2f}'.format(current_st_value.get())

    def get_current_bits_value():
        return '{: .2f}'.format(current_bits_value.get())
    ...
    run_button = tk.Button(
        ...
        input_entry.get(), output_entry.get(), 'INFO', int(float(get_current_bits_value())))
    )
